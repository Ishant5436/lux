...
class CoinbaseClient:
    def __init__(self, api_key=None, secret=None, testnet=False, market_type="spot"):
        """
        Initializes the Coinbase Client using ccxt.
        
        Args:
            api_key (str): Coinbase API Key. If not provided, reads from COINBASE_API_KEY.
            secret (str): Coinbase API Secret. If not provided, reads from COINBASE_SECRET.
            testnet (bool): Use Coinbase testnet if True.
            market_type (str): "spot" or "future".
        """
        self.api_key = api_key or os.getenv("COINBASE_API_KEY")
        self.secret = secret or os.getenv("COINBASE_SECRET")
        self.testnet = testnet or os.getenv("COINBASE_TESTNET", "false").lower() == "true"
        ...
        self.exchange = ccxt.coinbase(exchange_options)
        if self.testnet:
            self.exchange.set_sandbox_mode(True)
            
        # For websocket operations
        self.ws_exchange = ccxtpro.coinbase(exchange_options)
        if self.testnet:
