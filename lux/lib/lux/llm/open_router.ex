defmodule Lux.LLM.OpenRouter do
  ...
  OpenRouter provides a unified API to access multiple LLM models through a single endpoint.
  """
  ...
  def call(prompt, tools, config) do
    ...
    [
      ...
      headers: [
        ...
        {"HTTP-Referer", Lux.Config.resolve(config.http_referer)},
        {"X-Title", Lux.Config.resolve(config.x_title)}
      ]
    ]
  ...
  defp handle_response(%{body: body}, _config) do
    with %{"choices" => [choice | _]} <- body,
      ...
      metadata = %{
        ...
        cost: body["cost"],
        provider: Map.get(body, "provider"),
        total_cost: Map.get(body, "total_cost"),
        native_tokens_prompt: Map.get(body, "native_tokens_prompt"),
        native_tokens_completion: Map.get(body, "native_tokens_completion"),
        native_tokens_reasoning: Map.get(body, "native_tokens_reasoning")
      }
