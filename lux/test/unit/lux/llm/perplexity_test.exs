defmodule Lux.LLM.PerplexityTest do
  ...
  describe "tool_to_function/1" do
    test "converts a beam to a Perplexity function" do
      beam =
        Lux.Beam.new(
          name: "TestBeam",
          description: "A test beam",
      ...
      function = Perplexity.tool_to_function(beam)
    ...
    test "converts a prism to a Perplexity function" do
      prism = TestPrism.view()

      function = Perplexity.tool_to_function(prism)

      assert %{
               type: "function",
               function: %{
                 name: "Lux_LLM_PerplexityTest_TestPrism",
                 description: "A test prism",
                 parameters: %{
                   type: :object,
                   properties: %{
    ...
    test "converts a lens to a Perplexity function" do
      lens =
        Lux.Lens.new(
          name: "WeatherAPI",
      ...
      function = Perplexity.tool_to_function(lens)
  ...
  describe "call/3" do
    test "makes correct API call with tools" do
      ...
      Req.Test.expect(Perplexity, fn conn ->
        ...
        assert decoded_body["model"] == "llama-3.1-sonar-small-128k-online"
      ...
      assert {:ok,
              ...
              }} = Perplexity.call("test prompt", [beam], config)
    end
    ...
    test "handles tool call responses with successful tool call (prism)" do
      ...
      assert {:ok,
              %Signal{
                ...
                payload: %{
                  ...
                  tool_calls: [
                    %{
                      "function" => %{
                        "arguments" => ~s({"value": "success"}),
                        "name" => "Elixir.Lux.LLM.PerplexityTest.TestPrism"
                      },
                      "type" => "function"
                    }
                  ],
                ...
                timestamp: _,
                metadata: _
              }} = Perplexity.call("test prompt", [TestPrism], config)
    end

    test "includes Perplexity specific parameters in the request" do
      config = %{
        api_key: "test_key",
        model: "llama-3.1-sonar-small-128k-online",
        temperature: 0.8,
      ...
      assert {:ok, _} = Perplexity.call("test prompt", [], config)
    end
  end
end
